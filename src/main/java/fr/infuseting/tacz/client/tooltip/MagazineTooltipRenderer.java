package fr.infuseting.tacz.client.tooltip;
...
public class MagazineTooltipRenderer implements ClientTooltipComponent {
    ...
    public int getHeight() {
        ...
        if (!MagazineItem.isChecked(mag)) {
            return ICON_SIZE + PADDING * 2;
        }
        return ICON_SIZE + PADDING * 2 + BAR_HEIGHT + PADDING;
    }
    ...
    public void renderImage(Font font, int x, int y, GuiGraphics graphics) {
        ...
        boolean checked = MagazineItem.isChecked(mag);
        ...
        String countLine = getCountLine(mag);
        int countColor = (!checked || isEmpty) ? 0xAAAAAA : 0xFFD700;
    ...
    private static int getFillColor(int current, int max) {
        ...
        if (ratio > 0.6f) return 0xFF4CAF50;
        if (ratio > 0.3f) return 0xFFFFD700;
        return 0xFFE53935;
    }
}
