...
public class MagazineFamilySystem {
    ...
    // Isolated gun families â€” keyed by solo_ family ID, value is the capacity of that family
    ...
    public static void discoverMagazineFamilies() {
        ...
        for (Map.Entry<ResourceLocation, CommonGunIndex> entry : assetsManager.getAllGuns()) {
            ...
            // Extended families â€” one per level that meaningfully exceeds the base capacity
    ...
    private static boolean isCompatibleGun(CommonGunIndex index, ResourceLocation gunId) {
        ...
        if (reloadData.getType() != FeedType.MAGAZINE) {
            if (gunId != null) TaCZMagazines.LOGGER.debug("[mag-filter] SKIP {} â€” FeedType is {}", gunId, reloadData.getType());
            return false;
        }
        if (reloadData.isInfinite()) {
            if (gunId != null) TaCZMagazines.LOGGER.debug("[mag-filter] SKIP {} â€” infinite ammo", gunId);
            return false;
        }

        int ammoAmount = index.getGunData().getAmmoAmount();
        if (ammoAmount <= 2) {
            if (gunId != null) TaCZMagazines.LOGGER.debug("[mag-filter] SKIP {} â€” capacity {} â‰¤ 2", gunId, ammoAmount);
            return false;
        }
        ...
        if (emptyTime < 1.0f || tacticalTime < 1.0f) {
            if (gunId != null) TaCZMagazines.LOGGER.debug("[mag-filter] SKIP {} â€” reload times ({}/{}s) below 1s threshold", gunId, emptyTime, tacticalTime);
            return false;
        }
        ...
        // signal incremental loading â€” a broad endsWith("_feed") check incorrectly rejects
        ...
        if (scriptParams != null) {
            ...
            if (hasPerRoundFeed) {
                if (gunId != null) TaCZMagazines.LOGGER.debug("[mag-filter] SKIP {} â€” has per-round feed script params (loop_feed/round#_feed/clip_load_feed)", gunId);
                return false;
            }
        }
        ...
        // suffuse:python, ext [8,10,12] over a 6-round base) â€” which the old non-expandable-cylinder
        ...
        if (index.getGunData().getBolt() == Bolt.OPEN_BOLT) {
            ...
            if (!rapidFire) {
                if (gunId != null) TaCZMagazines.LOGGER.debug("[mag-filter] SKIP {} â€” revolver fingerprint (open-bolt + semi-only, {}-round)", gunId, ammoAmount);
                return false;
            }
        }

        if (gunId != null) TaCZMagazines.LOGGER.debug("[mag-filter] PASS {} â€” capacity={} empty={}s tactical={}s", gunId, ammoAmount, emptyTime, tacticalTime);
        return true;
    }
    ...
    // â”€â”€ Isolated gun families â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    public static void applyIsolatedGuns(java.util.Set<ResourceLocation> guns) {
        for (ResourceLocation gunId : guns) {
            ...
            if (sharedFamily == null) {
                TaCZMagazines.LOGGER.warn("[IsolatedGun] '{}' is not in any magazine family â€” skipping isolation", gunId);
                continue;
            }
            ...
            TaCZMagazines.LOGGER.info("[IsolatedGun] '{}' â†’ isolated family '{}'", gunId, soloId);
            ...
            if (extFamilies != null) {
                ...
                for (String extFamilyId : new ArrayList<>(extFamilies)) {
                    ...
                    TaCZMagazines.LOGGER.info("[IsolatedGun] '{}' â†’ isolated extended family '{}'", gunId, soloExtId);
                }
    ...
    // â”€â”€ Gun override / exclusion (applied by GunOverrideConfig after discovery) â”€â”€
    ...
    public static ResourceLocation getRepresentativeGun(String familyId) {
        ResourceLocation override = FamilyConfigManager.getOverride(familyId);
    ...
    public static Map<String, Set<Integer>> getAllMagazineFamiliesWithCapacities() {
        ...
        for (Map.Entry<String, Set<ResourceLocation>> entry : MAGAZINE_FAMILIES.entrySet()) {
            String familyId = entry.getKey();
            Set<ResourceLocation> guns = entry.getValue();
            if (guns == null || guns.isEmpty()) continue;
            if (EXTENDED_FAMILIES.contains(familyId)) continue;
            if (ISOLATED_BASE_FAMILIES.contains(familyId) || familyId.startsWith("solo_")) continue;
    ...
    // Parses the capacity from a family ID â€” handles base, extended, and isolated formats
    public static int getCapacityForFamily(String familyId) {
        ...
        if (EXTENDED_FAMILIES.contains(familyId)) {
            // format: ammoName_extCapacity_extN â€” strip _extN suffix first
    ...
    public static List<String> getFamiliesInCreativeTabOrder() {
        ...
        for (String ammoType : sortedAmmoTypes) {
            ...
            for (int capacity : sortedCapacities) {
                ...
                if (seen.add(familyId)) ordered.add(familyId);

                for (String extFamilyId : getExtendedFamiliesForBaseFamily(familyId)) {
                    if (seen.add(extFamilyId)) ordered.add(extFamilyId);
                }
            }
        }

        for (String soloFamilyId : getIsolatedBaseFamilies()) {
            if (seen.add(soloFamilyId)) ordered.add(soloFamilyId);
            for (String extFamilyId : getExtendedFamiliesForBaseFamily(soloFamilyId)) {
                if (seen.add(extFamilyId)) ordered.add(extFamilyId);
            }
        }
    ...
    public static String getFamilyDisplayName(String familyId) {
        // Isolated family: solo_namespace_path  â†’  "Namespace Path Solo Magazine"
